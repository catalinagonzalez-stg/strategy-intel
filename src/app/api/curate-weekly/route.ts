 ...
 * 1. Fetches promoted articles published or ingested in the last 14 days (+ pinned)
...
export async function POST() {
      try {
              ...
              lookbackDate.setDate(lookbackDate.getDate() - 14); // 14 days lookback

        // Gather promoted articles published or ingested in the last 14 days (or with no published_at)
        const { data: recentArticles } = await supabase
                .from('articles')
                .select('id, title, url, source_domain, content_snippet, published_at, notes, pinned, classifications(*)')
                .eq('status', 'promoted')
                .or(`published_at.gte.${lookbackDate.toISOString()},published_at.is.null,ingested_at.gte.${lookbackDate.toISOString()}`)
