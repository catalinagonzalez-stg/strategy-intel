 ...
 * - Article must be published OR ingested within the last 14 days (or have no published_at date)
 */
export async function POST(request: Request) {
  ...
  try {
              ...
              // Evaluate articles published OR ingested in the last 14 days, or with no published_at date
              ...
              const { data: articles } = await supabase
                    ...
                    .eq('status', 'new')
                    .or(`published_at.gte.${freshnessCutoff.toISOString()},published_at.is.null,ingested_at.gte.${freshnessCutoff.toISOString()}`)
